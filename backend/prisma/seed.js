const { PrismaClient, EstablishmentType, AvailabilityStatus } = require('@prisma/client');
...
async function main() {
  ...
  const existingEst = await prisma.establishment.count();
  if (existingEst === 0) {
  ...
  const existingWines = await prisma.wine.count();
  if (existingWines === 0) {
  ...
  console.log(`[Seed OK] Base de datos poblada con usuarios, ${establishments.length} tiendas y ${wines.length} vinos.`);
}
